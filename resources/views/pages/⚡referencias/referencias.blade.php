    ...
    <div class="mx-auto w-full max-w-7xl py-8">
        ...
        <div class="mb-8 flex flex-col sm:flex-row gap-3">
            <flux:input
                ...
                class="max-w-md"
            />
            <flux:select wire:model.live="type" placeholder="Todos os tipos" clearable class="sm:max-w-52">
        ...
        <div wire:loading.delay.flex class="hidden flex-wrap gap-4">
            @for ($i = 0; $i < 6; $i++)
        ...
        <div wire:loading.delay.remove class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <flux:modal name="add-material" class="md:w-[32rem]">
            <form wire:submit="addMaterial" class="space-y-5">
                ...
                <div class="flex gap-3">
                    <flux:select label="Tipo" wire:model="form.type" class="flex-1">
                        @foreach ($this->typeOptions as $case)
                            <flux:select.option value="{{ $case->value }}" icon="{{ $case->icon() }}">{{ $case->label() }}</flux:select.option>
                        @endforeach
                    </flux:select>
                ...
                <flux:input label="Editora" wire:model="form.publisher" placeholder="Opcional" />
